...
export class SlashCommandDropdown {
  ...
  private render(): void {
    ...
    if (this.filteredItems.length === 0) {
      const emptyEl = this.dropdownEl.createDiv({ cls: 'whalliam-slash-empty' });
      emptyEl.setText('No matching commands');
    } else {
      for (let i = 0; i < this.filteredItems.length; i++) {
        const item = this.filteredItems[i];
        const itemEl = this.dropdownEl.createDiv({ cls: 'whalliam-slash-item' });

        if (i === this.selectedIndex) {
          itemEl.addClass('selected');
        }

        const nameEl = itemEl.createSpan({ cls: 'whalliam-slash-name' });
        nameEl.setText(`${item.displayPrefix}${item.name}`);

        if (item.argumentHint) {
          const hintEl = itemEl.createSpan({ cls: 'whalliam-slash-hint' });
          hintEl.setText(normalizeArgumentHint(item.argumentHint));
        }

        if (item.description) {
          const descEl = itemEl.createDiv({ cls: 'whalliam-slash-desc' });
  ...
  private createDropdownElement(): HTMLElement {
    if (this.isFixed) {
      return this.containerEl.createDiv({
        cls: 'whalliam-slash-dropdown whalliam-slash-dropdown-fixed',
      });
    } else {
      return this.containerEl.createDiv({ cls: 'whalliam-slash-dropdown' });
    }
  }
  ...
  private positionFixed(): void {
    ...
    this.dropdownEl.setCssProps({
      '--whalliam-fixed-dropdown-bottom': `${window.innerHeight - inputRect.top + 4}px`,
      '--whalliam-fixed-dropdown-left': `${inputRect.left}px`,
      '--whalliam-fixed-dropdown-width': `${Math.max(inputRect.width, 280)}px`,
    });
  }
  ...
  private updateSelection(): void {
    const items = this.dropdownEl?.querySelectorAll('.whalliam-slash-item');
