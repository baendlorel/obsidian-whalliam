...
function createWikilink(
  ...
  link.className = 'whalliam-file-link internal-link';
...
function repairEmptyInternalLink(app: App, link: HTMLAnchorElement): void {
  ...
  link.classList.add('whalliam-file-link');
 ...
 * Handles both our custom .whalliam-file-link and Obsidian's .internal-link.
 */
export function registerFileLinkHandler(
  ...
  component.registerDomEvent(container, 'click', (event: MouseEvent) => {
    ...
    const link = target.closest('.whalliam-file-link, .internal-link') as HTMLAnchorElement;
  ...
  const walker = container.ownerDocument.createTreeWalker(
    ...
    NodeFilter.SHOW_TEXT,
    {
      acceptNode(node) {
        ...
        if (parent.closest('pre, code, a, .whalliam-file-link, .internal-link')) {
          return NodeFilter.FILTER_REJECT;
        }
