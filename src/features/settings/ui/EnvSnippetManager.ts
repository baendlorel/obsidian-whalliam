...
import type { WhalliamView } from '../../chat/WhalliamView';
...
export class EnvSnippetModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('whalliam-env-snippet-modal');
    ...
    const renderContextLimitFields = () => {
      ...
      if (uniqueModelIds.size === 0) {
        contextLimitsContainer.addClass('whalliam-hidden');
        return;
      }

      contextLimitsContainer.removeClass('whalliam-hidden');
      ...
      for (const modelId of uniqueModelIds) {
        const row = contextLimitsContainer.createDiv({ cls: 'whalliam-snippet-limit-row' });
        row.createSpan({ text: modelId, cls: 'whalliam-snippet-limit-model' });
        row.createSpan({ cls: 'whalliam-snippet-limit-spacer' });

        const aliasInput = row.createEl('input', {
          type: 'text',
          placeholder: t('settings.customModelAliases.placeholder'),
          cls: 'whalliam-snippet-alias-input',
        });
        ...
        const input = row.createEl('input', {
          type: 'text',
          placeholder: '200k',
          cls: 'whalliam-snippet-limit-input',
        });
    ...
    envVarsSetting.settingEl.addClass('whalliam-env-snippet-setting');
    envVarsSetting.controlEl.addClass('whalliam-env-snippet-control');

    contextLimitsContainer = contentEl.createDiv({ cls: 'whalliam-snippet-context-limits' });
    renderContextLimitFields();

    const buttonContainer = contentEl.createDiv({ cls: 'whalliam-snippet-buttons' });

    const cancelBtn = buttonContainer.createEl('button', {
      text: t('settings.envSnippets.modal.cancel'),
      cls: 'whalliam-cancel-btn'
    });
    cancelBtn.addEventListener('click', () => this.close());

    const saveBtn = buttonContainer.createEl('button', {
      text: this.snippet ? t('settings.envSnippets.modal.update') : t('settings.envSnippets.modal.save'),
      cls: 'whalliam-save-btn'
    });
...
export class EnvSnippetManager {
  ...
  private render() {
    ...
    const headerEl = this.containerEl.createDiv({ cls: 'whalliam-snippet-header' });
    headerEl.createSpan({ text: t('settings.envSnippets.name'), cls: 'whalliam-snippet-label' });

    const saveBtn = headerEl.createEl('button', {
      cls: 'whalliam-settings-action-btn',
    ...
    if (snippets.length === 0) {
      const emptyEl = this.containerEl.createDiv({ cls: 'whalliam-snippet-empty' });
      emptyEl.setText(t('settings.envSnippets.noSnippets'));
      return;
    }

    const listEl = this.containerEl.createDiv({ cls: 'whalliam-snippet-list' });

    for (const snippet of snippets) {
      const itemEl = listEl.createDiv({ cls: 'whalliam-snippet-item' });

      const infoEl = itemEl.createDiv({ cls: 'whalliam-snippet-info' });

      const nameEl = infoEl.createDiv({ cls: 'whalliam-snippet-name' });
      nameEl.setText(snippet.name);

      if (snippet.description) {
        const descEl = infoEl.createDiv({ cls: 'whalliam-snippet-description' });
        descEl.setText(snippet.description);
      }

      const actionsEl = itemEl.createDiv({ cls: 'whalliam-snippet-actions' });
      ...
      const deleteBtn = actionsEl.createEl('button', {
        cls: 'whalliam-settings-action-btn whalliam-settings-delete-btn',
  ...
  private async insertSnippet(snippet: EnvSnippet) {
    ...
    const view = this.plugin.app.workspace.getLeavesOfType('whalliam-view')[0]?.view as WhalliamView | undefined;
  ...
  private syncTextareaValue(scope: EnvironmentScope, value: string): void {
    const selector = `.whalliam-settings-env-textarea[data-env-scope="${scope}"]`;
