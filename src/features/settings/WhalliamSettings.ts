...
function addHotkeySettingRow(
  ...
  const item = containerEl.createDiv({ cls: 'whalliam-hotkey-item' });
  item.createSpan({
    cls: 'whalliam-hotkey-name',
    text: t(`${translationPrefix}.name` as TranslationKey),
  });
  if (hotkey) {
    item.createSpan({ cls: 'whalliam-hotkey-badge', text: hotkey });
  }
...
export class WhalliamSettingTab extends PluginSettingTab {
  ...
  constructor(app: App, plugin: WhalliamPlugin) {
    super(app, plugin);
    this.plugin = plugin;
  }
  ...
  display(): void {
    ...
    containerEl.empty();
    containerEl.addClass('whalliam-settings');
    ...
    const tabBar = containerEl.createDiv({ cls: 'whalliam-settings-tabs' });
    ...
    for (const id of tabIds) {
      ...
      const button = tabBar.createEl('button', {
        cls: `whalliam-settings-tab${id === this.activeTab ? ' whalliam-settings-tab--active' : ''}`,
        text: label,
      });
      button.addEventListener('click', () => {
        this.activeTab = id;
        for (const tabId of tabIds) {
          tabButtons.get(tabId)?.toggleClass('whalliam-settings-tab--active', tabId === id);
          tabContents.get(tabId)?.toggleClass('whalliam-settings-tab-content--active', tabId === id);
        }
      });
    ...
    for (const id of tabIds) {
      const content = containerEl.createDiv({
        cls: `whalliam-settings-tab-content${id === this.activeTab ? ' whalliam-settings-tab-content--active' : ''}`,
      });
  ...
  private renderGeneralTab(container: HTMLElement): void {
    ...
    const maxTabsWarningEl = container.createDiv({
      cls: 'whalliam-max-tabs-warning whalliam-setting-validation whalliam-setting-validation-warning whalliam-hidden',
    });
    maxTabsWarningEl.setText(t('settings.maxTabs.warning'));

    const updateMaxTabsWarning = (value: number): void => {
      maxTabsWarningEl.toggleClass('whalliam-hidden', value <= 5);
    };
    ...
    new Setting(container)
      ...
      .addText((text) => {
        ...
        text.inputEl.addClass('whalliam-settings-media-input');
        text.inputEl.addEventListener('blur', () => {
          void this.restartServiceForPromptChange();
        });
      });
    ...
    const hotkeyGrid = container.createDiv({ cls: 'whalliam-hotkey-grid' });
  ...
  private renderCustomContextLimits(container: HTMLElement, providerId?: ProviderId): void {
    ...
    const headerEl = container.createDiv({ cls: 'whalliam-context-limits-header' });
    headerEl.createSpan({
      text: t('settings.customModelOverrides.name'),
      cls: 'whalliam-context-limits-label',
    });

    const descEl = container.createDiv({ cls: 'whalliam-context-limits-desc' });
    descEl.setText(t('settings.customModelOverrides.desc'));

    const listEl = container.createDiv({ cls: 'whalliam-context-limits-list' });
    ...
    for (const modelId of uniqueModelIds) {
      ...
      const itemEl = listEl.createDiv({ cls: 'whalliam-context-limits-item' });
      const nameEl = itemEl.createDiv({ cls: 'whalliam-context-limits-model' });
      nameEl.setText(modelId);

      const inputWrapper = itemEl.createDiv({ cls: 'whalliam-context-limits-input-wrapper' });
      const aliasInputEl = inputWrapper.createEl('input', {
        type: 'text',
        placeholder: t('settings.customModelAliases.placeholder'),
        cls: 'whalliam-context-alias-input',
        value: currentAlias,
      });
      ...
      const inputEl = inputWrapper.createEl('input', {
        type: 'text',
        placeholder: '200k',
        cls: 'whalliam-context-limits-input',
        value: currentValue ? formatContextLimit(currentValue) : '',
      });
      inputEl.setAttribute('aria-label', `Context window for ${modelId}`);

      const validationEl = inputWrapper.createDiv({ cls: 'whalliam-context-limit-validation whalliam-hidden' });
      ...
      const saveContextLimit = async (): Promise<void> => {
        ...
        if (!trimmed) {
          ...
          validationEl.toggleClass('whalliam-hidden', true);
          inputEl.classList.remove('whalliam-input-error');
        } else {
          const parsed = parseContextLimit(trimmed);
          if (parsed === null) {
            validationEl.setText(t('settings.customContextLimits.invalid'));
            validationEl.toggleClass('whalliam-hidden', false);
            inputEl.classList.add('whalliam-input-error');
            return;
          }
