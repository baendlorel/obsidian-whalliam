...
export function buildInlineEditInputDecorations(options: {
  ...
  return Decoration.set([
    Decoration.line({
      class: 'whalliam-inline-input-line',
    }).range(lineStart),
    Decoration.widget({
...
function getDiffBlockClass(type: DiffOp['type']): string {
  switch (type) {
    case 'delete':
      return 'whalliam-diff-del';
    case 'insert':
      return 'whalliam-diff-ins';
    default:
      return 'whalliam-diff-equal';
  }
}
...
class InlineEditController {
  ...
  createInputDOM(): HTMLElement {
    ...
    container.className = 'whalliam-inline-input-container';
    this.containerEl = container;

    this.agentReplyEl = ownerDocument.createElement('div');
    this.agentReplyEl.className = 'whalliam-inline-agent-reply whalliam-hidden';
    container.appendChild(this.agentReplyEl);

    const inputWrap = ownerDocument.createElement('div');
    inputWrap.className = 'whalliam-inline-input-wrap';
    container.appendChild(inputWrap);

    this.inputEl = ownerDocument.createElement('input');
    this.inputEl.type = 'text';
    this.inputEl.className = 'whalliam-inline-input';
    ...
    this.spinnerEl.className = 'whalliam-inline-spinner whalliam-hidden';
  ...
  createDiffPreviewDOM(diffOps: DiffOp[]): HTMLElement {
    ...
    previewEl.className = 'whalliam-inline-diff-preview';

    const bodyEl = ownerDocument.createElement('div');
    bodyEl.className = 'whalliam-inline-diff-preview-body markdown-rendered';
    previewEl.appendChild(bodyEl);

    const actionsEl = ownerDocument.createElement('div');
    actionsEl.className = 'whalliam-inline-preview-actions';
  ...
  private createPreviewActionButton(
    ...
    button.type = 'button';
    button.className = `whalliam-inline-preview-action ${variant}`;
  ...
  private async renderMarkdownDiffPreview(container: HTMLElement, diffOps: DiffOp[]): Promise<void> {
    ...
    for (const document of buildMarkdownDiffDocuments(diffOps)) {
      ...
      opEl.className = `whalliam-diff-block ${getDiffBlockClass(document.type)}`;
  ...
  private async generate() {
    ...
    this.spinnerEl.removeClass('whalliam-hidden');
    ...
    this.spinnerEl.addClass('whalliam-hidden');
  ...
  private showAgentReply(message: string) {
    ...
    replyEl.removeClass('whalliam-hidden');
    replyEl.empty();
