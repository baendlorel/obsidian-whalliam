...
export function createWriteEditBlock(
  ...
  const wrapperEl = parentEl.createDiv({ cls: 'whalliam-write-edit-block' });
  wrapperEl.dataset.toolId = toolCall.id;

  // Header (clickable to collapse/expand)
  const headerEl = wrapperEl.createDiv({ cls: 'whalliam-write-edit-header' });
  headerEl.setAttribute('tabindex', '0');
  headerEl.setAttribute('role', 'button');

  // File icon
  const iconEl = headerEl.createDiv({ cls: 'whalliam-write-edit-icon' });
  iconEl.setAttribute('aria-hidden', 'true');
  setIcon(iconEl, getToolIcon(toolName));

  const nameEl = headerEl.createDiv({ cls: 'whalliam-write-edit-name' });
  nameEl.setText(toolName);
  const summaryEl = headerEl.createDiv({ cls: 'whalliam-write-edit-summary' });
  summaryEl.setText(fileNameOnly(filePath) || 'file');

  // Populated when diff is computed
  const statsEl = headerEl.createDiv({ cls: 'whalliam-write-edit-stats' });

  const statusEl = headerEl.createDiv({ cls: 'whalliam-write-edit-status status-running' });
  statusEl.setAttribute('aria-label', 'Status: running');

  // Content area (collapsed by default)
  const contentEl = wrapperEl.createDiv({ cls: 'whalliam-write-edit-content' });

  // Initial loading state
  const loadingRow = contentEl.createDiv({ cls: 'whalliam-write-edit-diff-row' });
  const loadingEl = loadingRow.createDiv({ cls: 'whalliam-write-edit-loading' });
...
export function updateWriteEditWithDiff(state: WriteEditState, diffData: ToolDiffData): void {
  ...
  // Render diff content
  const row = state.contentEl.createDiv({ cls: 'whalliam-write-edit-diff-row' });
  const diffEl = row.createDiv({ cls: 'whalliam-write-edit-diff' });
...
export function finalizeWriteEditBlock(state: WriteEditState, isError: boolean): void {
  ...
  state.statusEl.className = 'whalliam-write-edit-status';
  ...
  if (isError) {
    ...
    if (!state.diffLines) {
      state.contentEl.empty();
      const row = state.contentEl.createDiv({ cls: 'whalliam-write-edit-diff-row' });
      const errorEl = row.createDiv({ cls: 'whalliam-write-edit-error' });
    ...
    state.contentEl.empty();
    const row = state.contentEl.createDiv({ cls: 'whalliam-write-edit-diff-row' });
    const doneEl = row.createDiv({ cls: 'whalliam-write-edit-done-text' });
    doneEl.setText('DONE');
  }
...
export function renderStoredWriteEdit(
  ...
  const statusEl = headerEl.createDiv({ cls: 'whalliam-write-edit-status' });
  if (isError) {
  ...
  const row = contentEl.createDiv({ cls: 'whalliam-write-edit-diff-row' });
