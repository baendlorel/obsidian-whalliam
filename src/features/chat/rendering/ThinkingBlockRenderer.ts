...
export function createThinkingBlock(
  ...
  const wrapperEl = parentEl.createDiv({ cls: 'whalliam-thinking-block' });

  // Header (clickable to expand/collapse)
  const header = wrapperEl.createDiv({ cls: 'whalliam-thinking-header' });
  ...
  // Label with timer
  const labelEl = header.createSpan({ cls: 'whalliam-thinking-label' });
  ...
  const contentEl = wrapperEl.createDiv({ cls: 'whalliam-thinking-content' });
...
export function finalizeThinkingBlock(state: ThinkingBlockState): number {
  ...
  const header = state.wrapperEl.querySelector('.whalliam-thinking-header');
  if (header) {
