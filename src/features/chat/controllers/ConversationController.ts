...
export class ConversationController {
  ...
  async createNew(options: { force?: boolean } = {}): Promise<void> {
    ...
    try {
      ...
      welcomeEl.createDiv({ cls: 'whalliam-welcome-greeting', text: this.getGreeting() });
  ...
  private renderHistoryItems(
    ...
    const dropdownHeader = container.createDiv({ cls: 'whalliam-history-header' });
    dropdownHeader.createSpan({ text: 'Conversations' });

    const list = container.createDiv({ cls: 'whalliam-history-list' });
    const allConversations = plugin.getConversationList();

    if (allConversations.length === 0) {
      list.createDiv({ cls: 'whalliam-history-empty', text: 'No conversations' });
      return;
    }
    ...
    for (const conv of conversations) {
      ...
      const iconEl = item.createDiv({ cls: 'whalliam-history-item-icon' });
      setIcon(iconEl, this.getHistoryItemIcon(openState, isRunning));

      const content = item.createDiv({ cls: 'whalliam-history-item-content' });
      const titleEl = content.createDiv({ cls: 'whalliam-history-item-title', text: conv.title });
      titleEl.setAttribute('title', conv.title);
      content.createDiv({
        cls: 'whalliam-history-item-date',
      ...
      const actions = item.createDiv({ cls: 'whalliam-history-item-actions' });

      // Show regenerate button if title generation failed, or loading indicator if pending
      if (conv.titleGenerationStatus === 'pending') {
        const loadingEl = actions.createEl('span', { cls: 'whalliam-action-btn whalliam-action-loading' });
        setIcon(loadingEl, 'loader-2');
        loadingEl.setAttribute('aria-label', 'Generating title...');
      } else if (conv.titleGenerationStatus === 'failed') {
        const regenerateBtn = actions.createEl('button', { cls: 'whalliam-action-btn' });
      ...
      if (openState === 'closed' && options.onOpenConversationInNewTab) {
        const openInNewTabBtn = actions.createEl('button', {
          cls: 'whalliam-action-btn whalliam-open-new-tab-btn',
        });
      ...
      const renameBtn = actions.createEl('button', { cls: 'whalliam-action-btn' });
      ...
      const deleteBtn = actions.createEl('button', { cls: 'whalliam-action-btn whalliam-delete-btn' });
  ...
  private showRenameInput(item: HTMLElement, convId: string, currentTitle: string): void {
    const titleEl = item.querySelector('.whalliam-history-item-title') as HTMLElement;
    if (!titleEl) return;

    const input = (item.ownerDocument ?? window.document).createElement('input');
    input.type = 'text';
    input.className = 'whalliam-rename-input';
  ...
  getGreeting(): string {
    ...
    const getTimeGreetings = (): string[] => {
      if (hour >= 5 && hour < 12) {
        return [personalize('Good morning'), 'Coffee and Whalliam time?'];
  ...
  updateWelcomeVisibility(): void {
    ...
    if (this.deps.state.messages.length === 0) {
      welcomeEl.removeClass('whalliam-hidden');
    } else {
      welcomeEl.addClass('whalliam-hidden');
    }
  }
  ...
  initializeWelcome(): void {
    ...
    if (!welcomeEl.querySelector('.whalliam-welcome-greeting')) {
      welcomeEl.createDiv({ cls: 'whalliam-welcome-greeting', text: this.getGreeting() });
    }
  ...
  // ============================================
  // History Dropdown Rendering (for WhalliamView)
  // ============================================

  /**
   * Renders the history dropdown content to a provided container.
   * Used by WhalliamView to render the dropdown with custom selection callback.
   */
