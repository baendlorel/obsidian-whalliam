...
export class InputController {
  ...
  async sendMessage(options?: {
    ...
    streamController.showThinkingIndicator(
      ...
      isCompact ? 'whalliam-thinking--compact' : undefined,
    );
        ...
        if (!didCancelThisTurn && !hasCompactBoundary) {
          ...
          if (durationSeconds > 0) {
            ...
            if (state.currentContentEl) {
              const footerEl = state.currentContentEl.createDiv({ cls: 'whalliam-response-footer' });
              footerEl.createSpan({
  ...
  updateQueueIndicator(): void {
    ...
    if (visibleQueuedMessage) {
      ...
      indicatorEl.addClass('whalliam-visible-flex');
      indicatorEl.removeClass('whalliam-hidden');
      return;
    }

    indicatorEl.removeClass('whalliam-visible-flex');
    indicatorEl.addClass('whalliam-hidden');
  }
  ...
  private createQueueIconButton(
    ...
    const button = parentEl.createEl('button', {
      cls: 'whalliam-queue-indicator-icon-action',
      attr: {
        'aria-label': label,
        title: label,
        type: 'button',
  ...
  private rollbackFailedTurn(
    ...
    if (messagesBeforeTurn.length === 0) {
      this.deps.getWelcomeEl()?.removeClass('whalliam-hidden');
    }
  }
  ...
  async handleApprovalRequest(
    ...
    const headerEl = parentEl.createDiv({ cls: 'whalliam-ask-approval-info' });
    headerEl.remove();

    const toolEl = headerEl.createDiv({ cls: 'whalliam-ask-approval-tool' });
    const iconEl = toolEl.createSpan({ cls: 'whalliam-ask-approval-icon' });
    iconEl.setAttribute('aria-hidden', 'true');
    setToolIcon(iconEl, toolName);
    toolEl.createSpan({ text: toolName, cls: 'whalliam-ask-approval-tool-name' });

    if (approvalOptions?.decisionReason) {
      headerEl.createDiv({ text: approvalOptions.decisionReason, cls: 'whalliam-ask-approval-reason' });
    }
    if (approvalOptions?.blockedPath) {
      headerEl.createDiv({ text: approvalOptions.blockedPath, cls: 'whalliam-ask-approval-blocked-path' });
    }
    if (approvalOptions?.agentID) {
      headerEl.createDiv({ text: `Agent: ${approvalOptions.agentID}`, cls: 'whalliam-ask-approval-agent' });
    }

    headerEl.createDiv({ text: description, cls: 'whalliam-ask-approval-desc' });
  ...
  private hideInputContainer(inputContainerEl: HTMLElement): void {
    ...
    inputContainerEl.addClass('whalliam-hidden');
  }
  ...
  private restoreInputContainer(inputContainerEl: HTMLElement): void {
    ...
    if (this.inputContainerHideDepth === 0) {
      inputContainerEl.removeClass('whalliam-hidden');
    }
  }
  ...
  private resetInputContainerVisibility(): void {
    if (this.inputContainerHideDepth > 0) {
      ...
      this.deps.getInputContainerEl().removeClass('whalliam-hidden');
    }
  }
