...
export class StreamController {
  ...
  private handleRegularToolUse(
    ...
    if (existingToolCall) {
      ...
      if (Object.keys(newInput).length > 0) {
        ...
        if (toolEl) {
          const nameEl = toolEl.querySelector('.whalliam-tool-name')
            ?? toolEl.querySelector('.whalliam-write-edit-name');
          if (nameEl) {
            nameEl.setText(getToolName(existingToolCall.name, existingToolCall.input));
          }
          const summaryEl = toolEl.querySelector('.whalliam-tool-summary')
            ?? toolEl.querySelector('.whalliam-write-edit-summary');
          if (summaryEl) {
  ...
  async appendText(text: string): Promise<void> {
    ...
    if (!state.currentTextEl) {
      state.currentTextEl = state.currentContentEl.createDiv({ cls: 'whalliam-text-block' });
  ...
  showThinkingIndicator(overrideText?: string, overrideCls?: string): void {
    ...
    state.setThinkingIndicatorTimeout(timerWindow.setTimeout(() => {
      ...
      const timerSpan = state.thinkingEl.createSpan({ cls: 'whalliam-thinking-hint' });
      const updateTimer = () => {
  ...
  private renderCompactBoundary(): void {
    ...
    if (!state.currentContentEl) return;
    this.hideThinkingIndicator();
    const el = state.currentContentEl.createDiv({ cls: 'whalliam-compact-boundary' });
    el.createSpan({ cls: 'whalliam-compact-boundary-label', text: 'Conversation compacted' });
  }
