...
import { VIEW_TYPE_WHALLIAM } from '../../core/types';
...
export class WhalliamView extends ItemView {
  ...
  constructor(leaf: WorkspaceLeaf, plugin: WhalliamPlugin) {
    ...
    // overwritten by prototype patching. Hover Editor patches WhalliamView.prototype.load
  ...
  async onOpen() {
    ...
    this.viewContainerEl.addClass('whalliam-container');

    const header = this.viewContainerEl.createDiv({ cls: 'whalliam-header' });
    this.buildHeader(header);

    this.navRowContent = this.buildNavRowContent();
    this.tabContentEl = this.viewContainerEl.createDiv({ cls: 'whalliam-tab-content-container' });
  ...
  private buildHeader(header: HTMLElement): void {
    const titleEl = header.createDiv({ cls: 'whalliam-title' });
  ...
  private buildNavRowContent(): HTMLElement {
    ...
    this.tabBarContainerEl.className = 'whalliam-tab-bar-container';
    ...
    navActionsEl.className = 'whalliam-input-nav-actions';

    this.newTabButtonEl = navActionsEl.createDiv({ cls: 'whalliam-input-nav-btn whalliam-new-tab-btn' });
    ...
    const newBtn = navActionsEl.createDiv({ cls: 'whalliam-input-nav-btn' });
    ...
    // History dropdown
    const historyContainer = navActionsEl.createDiv({ cls: 'whalliam-history-container' });
    const historyBtn = historyContainer.createDiv({ cls: 'whalliam-input-nav-btn' });
    setIcon(historyBtn, 'history');
    historyBtn.setAttribute('aria-label', 'Chat history');

    this.historyDropdown = historyContainer.createDiv({ cls: 'whalliam-history-menu' });
    ...
    wrapper.className = 'whalliam-input-nav-content';
  ...
  private buildInputFooter(): void {
    ...
    this.inputFooterEl = this.viewContainerEl.createDiv({ cls: 'whalliam-input-footer' });
    this.inputNavRowHostEl = this.inputFooterEl.createDiv({
      cls: 'whalliam-input-nav-row whalliam-view-input-nav-row',
    });
    this.activeInputSlotEl = this.inputFooterEl.createDiv({ cls: 'whalliam-active-input-slot' });
  }
  ...
  private updateTabBarVisibility(): void {
    ...
    this.tabBarContainerEl.toggleClass('whalliam-hidden', !showTabBar);
  ...
  private updateNewTabButtonVisibility(): void {
    ...
    this.newTabButtonEl.toggleClass('whalliam-hidden', !canCreateTab);
    if (canCreateTab) {
