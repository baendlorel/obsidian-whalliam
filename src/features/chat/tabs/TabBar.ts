...
export class TabBar {
  ...
  private build(): void {
    this.containerEl.addClass('whalliam-tab-badges');
  ...
  private renderBadge(item: TabBarItem): void {
    ...
    let stateClass = 'whalliam-tab-badge-idle';
    if (item.isActive) {
      stateClass = 'whalliam-tab-badge-active';
    } else if (item.needsAttention) {
      stateClass = 'whalliam-tab-badge-attention';
    } else if (item.isStreaming) {
      stateClass = 'whalliam-tab-badge-streaming';
    }
    ...
    const badgeEl = this.containerEl.createDiv({
      cls: [
        'whalliam-tab-badge',
        stateClass,
        isTitleExpanded ? 'whalliam-tab-badge-expanded' : '',
      ].filter(Boolean).join(' '),
  ...
  destroy(): void {
    this.containerEl.empty();
    this.containerEl.removeClass('whalliam-tab-badges');
  ...
  private toggleBadgeTitle(item: TabBarItem, badgeEl: HTMLElement): void {
    ...
    badgeEl.toggleClass('whalliam-tab-badge-expanded', isTitleExpanded);
    badgeEl.setAttribute('data-title-expanded', isTitleExpanded ? 'true' : 'false');
