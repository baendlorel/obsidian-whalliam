...
import type { ChatMessage, WhalliamSettings, Conversation, StreamChunk } from '../../../core/types';
...
function shouldSendMessageFromEnterKey(
  e: KeyboardEvent,
  settings: Pick<WhalliamSettings, 'requireCommandOrControlEnterToSend'>,
): boolean {
...
function refreshTabProviderUI(tab: TabData, plugin: WhalliamPlugin): void {
 ...
 */
function applyProviderUIGating(tab: TabData, plugin: WhalliamPlugin): void {
  const capabilities = getTabCapabilities(tab, plugin);
...
function ensureTitleGenerationService(tab: TabData, plugin: WhalliamPlugin): void {
 ...
 */
export function onProviderAvailabilityChanged(tab: TabData, plugin: WhalliamPlugin): void {
...
export function createTab(options: TabCreateOptions): TabData {
  ...
  const contentEl = containerEl.createDiv({ cls: 'whalliam-tab-content whalliam-hidden' });
...
function buildTabDOM(contentEl: HTMLElement): TabDOMElements {
  const messagesWrapperEl = contentEl.createDiv({ cls: 'whalliam-messages-wrapper' });
  const messagesEl = messagesWrapperEl.createDiv({ cls: 'whalliam-messages' });
  const welcomeEl = messagesEl.createDiv({ cls: 'whalliam-welcome' });
  const statusPanelContainerEl = contentEl.createDiv({ cls: 'whalliam-status-panel-container' });
  const inputComposerEl = contentEl.createDiv({ cls: 'whalliam-input-composer' });
  const inputContainerEl = inputComposerEl.createDiv({ cls: 'whalliam-input-container' });
  const queueIndicatorEl = inputContainerEl.createDiv({ cls: 'whalliam-input-queue-row' });
  const navRowEl = inputContainerEl.createDiv({ cls: 'whalliam-input-nav-row' });
  const inputWrapper = inputContainerEl.createDiv({ cls: 'whalliam-input-wrapper' });
  const contextRowEl = inputWrapper.createDiv({ cls: 'whalliam-context-row' });
...
function initializeContextManagers(tab: TabData, plugin: WhalliamPlugin): void {
  const { dom } = tab;
 ...
 */
function initializeInstructionAndTodo(tab: TabData, plugin: WhalliamPlugin): void {
  const { dom } = tab;
...
function initializeInputToolbar(
  ...
  const inputToolbar = dom.inputWrapper.createDiv({ cls: 'whalliam-input-toolbar' });
...
export function initializeTabUI(
  ...
  dom.selectionIndicatorEl = dom.contextRowEl.createDiv({ cls: 'whalliam-selection-indicator whalliam-hidden' });

  dom.browserIndicatorEl = dom.contextRowEl.createDiv({ cls: 'whalliam-browser-selection-indicator whalliam-hidden' });

  dom.canvasIndicatorEl = dom.contextRowEl.createDiv({ cls: 'whalliam-canvas-indicator whalliam-hidden' });
 ...
 */
function resolveForkSource(tab: TabData, plugin: WhalliamPlugin): ForkSource | null {
 ...
 */
export function wireTabInputEvents(tab: TabData, plugin: WhalliamPlugin): void {
...
export function activateTab(tab: TabData): void {
  tab.dom.contentEl.removeClass('whalliam-hidden');
...
export function deactivateTab(tab: TabData): void {
  tab.dom.contentEl.addClass('whalliam-hidden');
 ...
 */
export function getTabTitle(tab: TabData, plugin: WhalliamPlugin): string {
  if (tab.conversationId) {
...
export function setupServiceCallbacks(tab: TabData, plugin: WhalliamPlugin): void {
...
async function renderAutoTriggeredTurn(tab: TabData, result: AutoTurnResult): Promise<void> {
  ...
  if (hasVisibleContent) {
    ...
    const contentEl = msgEl?.querySelector<HTMLElement>('.whalliam-message-content');
    if (contentEl) {
...
export function updatePlanModeUI(tab: TabData, plugin: WhalliamPlugin, mode: string): void {
  const providerId = getTabProviderId(tab, plugin);
  const snapshot = getWritableTabSettingsSnapshot(tab, plugin);
  const uiConfig = ProviderRegistry.getChatUIConfig(providerId);
  if (uiConfig.applyPermissionMode) {
