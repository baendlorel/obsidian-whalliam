...
export class ImageContextManager {
  ...
  constructor(
    ...
    const fileIndicator = this.previewContainerEl.querySelector('.whalliam-file-indicator');
    this.imagePreviewEl = this.previewContainerEl.createDiv({ cls: 'whalliam-image-preview' });
  ...
  private setupDragAndDrop() {
    const inputWrapper = this.containerEl.querySelector('.whalliam-input-wrapper') as HTMLElement;
    if (!inputWrapper) return;

    this.dropOverlay = inputWrapper.createDiv({ cls: 'whalliam-drop-overlay' });
    const dropContent = this.dropOverlay.createDiv({ cls: 'whalliam-drop-content' });
  ...
  private handleDragLeave(e: DragEvent) {
    ...
    const inputWrapper = this.containerEl.querySelector('.whalliam-input-wrapper');
    if (!inputWrapper) {
  ...
  private updateImagePreview() {
    ...
    if (this.attachedImages.size === 0) {
      this.imagePreviewEl.removeClass('whalliam-visible-flex');
      this.imagePreviewEl.addClass('whalliam-hidden');
      return;
    }

    this.imagePreviewEl.addClass('whalliam-visible-flex');
    this.imagePreviewEl.removeClass('whalliam-hidden');
  ...
  private renderImagePreview(id: string, image: ImageAttachment) {
    const previewEl = this.imagePreviewEl.createDiv({ cls: 'whalliam-image-chip' });

    const thumbEl = previewEl.createDiv({ cls: 'whalliam-image-thumb' });
    ...
    const infoEl = previewEl.createDiv({ cls: 'whalliam-image-info' });
    const nameEl = infoEl.createSpan({ cls: 'whalliam-image-name' });
    nameEl.setText(this.truncateName(image.name, 20));
    nameEl.setAttribute('title', image.name);

    const sizeEl = infoEl.createSpan({ cls: 'whalliam-image-size' });
    sizeEl.setText(this.formatSize(image.size));

    const removeEl = previewEl.createSpan({ cls: 'whalliam-image-remove' });
    removeEl.setText('\u00D7');
