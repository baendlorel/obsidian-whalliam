...
export class ModelSelector {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    ...
    this.container = parentEl.createDiv({ cls: 'whalliam-model-selector' });
    this.render();
  }
  ...
  private render() {
    ...
    this.buttonEl = this.container.createDiv({ cls: 'whalliam-model-btn' });
    this.updateDisplay();

    this.dropdownEl = this.container.createDiv({ cls: 'whalliam-model-dropdown' });
    this.renderOptions();
  }
  ...
  updateDisplay() {
    ...
    const labelEl = this.buttonEl.createSpan({ cls: 'whalliam-model-label' });
  ...
  renderOptions() {
    ...
    for (const model of reversed) {
      if (model.group && model.group !== lastGroup) {
        const separator = this.dropdownEl.createDiv({ cls: 'whalliam-model-group' });
        separator.setText(model.group);
        lastGroup = model.group;
      }

      const option = this.dropdownEl.createDiv({ cls: 'whalliam-model-option' });
      ...
      if (icon) {
        option.appendChild(createProviderIconSvg(icon, {
          className: 'whalliam-model-provider-icon',
          height: 12,
...
export class ModeSelector {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    this.callbacks = callbacks;
    this.container = parentEl.createDiv({ cls: 'whalliam-mode-selector' });
    this.render();
  }
  ...
  private render() {
    ...
    this.labelEl = this.container.createSpan({ cls: 'whalliam-mode-label' });
    this.toggleEl = this.container.createDiv({ cls: 'whalliam-toggle-switch' });
  ...
  updateDisplay() {
    ...
    if (!selectorConfig || selectorConfig.options.length !== 2) {
      this.container.addClass('whalliam-hidden');
      return;
    }

    this.container.removeClass('whalliam-hidden');
...
export class ThinkingBudgetSelector {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    this.callbacks = callbacks;
    this.container = parentEl.createDiv({ cls: 'whalliam-thinking-selector' });
    this.render();
  }
  ...
  private render() {
    ...
    this.effortEl = this.container.createDiv({ cls: 'whalliam-thinking-effort' });
    const effortLabel = this.effortEl.createSpan({ cls: 'whalliam-thinking-label-text' });
    effortLabel.setText('Effort:');
    this.effortGearsEl = this.effortEl.createDiv({ cls: 'whalliam-thinking-gears' });

    // Legacy budget selector (for custom models)
    this.budgetEl = this.container.createDiv({ cls: 'whalliam-thinking-budget' });
    const budgetLabel = this.budgetEl.createSpan({ cls: 'whalliam-thinking-label-text' });
    budgetLabel.setText('Thinking:');
    this.budgetGearsEl = this.budgetEl.createDiv({ cls: 'whalliam-thinking-gears' });
  ...
  private renderEffortGears() {
    ...
    const currentEl = this.effortGearsEl.createDiv({ cls: 'whalliam-thinking-current' });
    currentEl.setText(currentInfo?.label || options[0]?.label || 'High');

    const optionsEl = this.effortGearsEl.createDiv({ cls: 'whalliam-thinking-options' });

    for (const effort of [...options].reverse()) {
      const gearEl = optionsEl.createDiv({ cls: 'whalliam-thinking-gear' });
  ...
  private renderBudgetGears() {
    ...
    const currentEl = this.budgetGearsEl.createDiv({ cls: 'whalliam-thinking-current' });
    currentEl.setText(currentBudgetInfo?.label || options[0]?.label || 'Off');

    const optionsEl = this.budgetGearsEl.createDiv({ cls: 'whalliam-thinking-options' });
  ...
  updateDisplay() {
    ...
    if (capabilities.reasoningControl === 'none') {
      this.effortEl?.addClass('whalliam-hidden');
      this.budgetEl?.addClass('whalliam-hidden');
      return;
    }
    ...
    if (this.effortEl) {
      this.effortEl.toggleClass('whalliam-hidden', !adaptive);
    }
    if (this.budgetEl) {
      this.budgetEl.toggleClass('whalliam-hidden', adaptive);
    }
...
export class PermissionToggle {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    this.callbacks = callbacks;
    this.container = parentEl.createDiv({ cls: 'whalliam-permission-toggle' });
    this.render();
  }
  ...
  private render() {
    ...
    this.labelEl = this.container.createSpan({ cls: 'whalliam-permission-label' });
    this.toggleEl = this.container.createDiv({ cls: 'whalliam-toggle-switch' });
  ...
  updateDisplay() {
    ...
    if (canShowPlan && planValue && mode === planValue) {
      this.toggleEl.addClass('whalliam-hidden');
      this.labelEl.setText(planLabel);
      this.labelEl.addClass('plan-active');
    } else {
      this.toggleEl.removeClass('whalliam-hidden');
...
export class ServiceTierToggle {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    this.callbacks = callbacks;
    this.container = parentEl.createDiv({ cls: 'whalliam-service-tier-toggle' });
    this.render();
  }
  ...
  private render() {
    ...
    this.buttonEl = this.container.createDiv({ cls: 'whalliam-service-tier-button' });
    this.iconEl = this.buttonEl.createSpan({ cls: 'whalliam-service-tier-icon' });
...
export class ExternalContextSelector {
  ...
  constructor(parentEl: HTMLElement, callbacks: ToolbarCallbacks) {
    this.callbacks = callbacks;
    this.container = parentEl.createDiv({ cls: 'whalliam-external-context-selector' });
    this.render();
  }
  ...
  private render() {
    ...
    const iconWrapper = this.container.createDiv({ cls: 'whalliam-external-context-icon-wrapper' });

    this.iconEl = iconWrapper.createDiv({ cls: 'whalliam-external-context-icon' });
    setIcon(this.iconEl, 'folder');

    this.badgeEl = iconWrapper.createDiv({ cls: 'whalliam-external-context-badge' });
    ...
    this.dropdownEl = this.container.createDiv({ cls: 'whalliam-external-context-dropdown' });
    this.renderDropdown();
  }
  ...
  private renderDropdown() {
    ...
    // Header
    const headerEl = this.dropdownEl.createDiv({ cls: 'whalliam-external-context-header' });
    headerEl.setText('External contexts');

    // Path list
    const listEl = this.dropdownEl.createDiv({ cls: 'whalliam-external-context-list' });

    if (this.externalContextPaths.length === 0) {
      const emptyEl = listEl.createDiv({ cls: 'whalliam-external-context-empty' });
      emptyEl.setText('Click folder icon to add');
    } else {
      for (const pathStr of this.externalContextPaths) {
        const itemEl = listEl.createDiv({ cls: 'whalliam-external-context-item' });

        const pathTextEl = itemEl.createSpan({ cls: 'whalliam-external-context-text' });
        ...
        const lockBtn = itemEl.createSpan({ cls: 'whalliam-external-context-lock' });
        if (isPersistent) {
        ...
        const removeBtn = itemEl.createSpan({ cls: 'whalliam-external-context-remove' });
        setIcon(removeBtn, 'x');
...
export class McpServerSelector {
  ...
  constructor(parentEl: HTMLElement) {
    this.container = parentEl.createDiv({ cls: 'whalliam-mcp-selector' });
    this.render();
  }
  ...
  private render() {
    ...
    const iconWrapper = this.container.createDiv({ cls: 'whalliam-mcp-selector-icon-wrapper' });

    this.iconEl = iconWrapper.createDiv({ cls: 'whalliam-mcp-selector-icon' });
    appendMcpIcon(this.iconEl);

    this.badgeEl = iconWrapper.createDiv({ cls: 'whalliam-mcp-selector-badge' });

    this.updateDisplay();

    this.dropdownEl = this.container.createDiv({ cls: 'whalliam-mcp-selector-dropdown' });
    this.renderDropdown();
  ...
  private renderDropdown() {
    ...
    // Header
    const headerEl = this.dropdownEl.createDiv({ cls: 'whalliam-mcp-selector-header' });
    headerEl.setText('Mcp servers');

    // Server list
    const listEl = this.dropdownEl.createDiv({ cls: 'whalliam-mcp-selector-list' });

    const allServers = this.mcpManager?.getServers() || [];
    const servers = allServers.filter(s => s.enabled);

    if (servers.length === 0) {
      const emptyEl = listEl.createDiv({ cls: 'whalliam-mcp-selector-empty' });
  ...
  private renderServerItem(listEl: HTMLElement, server: ManagedMcpServer) {
    const itemEl = listEl.createDiv({ cls: 'whalliam-mcp-selector-item' });
    ...
    // Checkbox
    const checkEl = itemEl.createDiv({ cls: 'whalliam-mcp-selector-check' });
    if (isEnabled) {
      appendCheckIcon(checkEl);
    }

    // Info
    const infoEl = itemEl.createDiv({ cls: 'whalliam-mcp-selector-item-info' });

    const nameEl = infoEl.createSpan({ cls: 'whalliam-mcp-selector-item-name' });
    nameEl.setText(server.name);

    // Badges
    if (server.contextSaving) {
      const csEl = infoEl.createSpan({ cls: 'whalliam-mcp-selector-cs-badge' });
      csEl.setText('@');
  ...
  private toggleServer(name: string, itemEl: HTMLElement) {
    ...
    const checkEl = itemEl.querySelector<HTMLElement>('.whalliam-mcp-selector-check');
...
export class ContextUsageMeter {
  ...
  constructor(parentEl: HTMLElement) {
    this.container = parentEl.createDiv({ cls: 'whalliam-context-meter' });
    this.render();
    // Initially hidden
    this.container.addClass('whalliam-hidden');
  }
  ...
  setVisible(visible: boolean): void {
    this.container.toggleClass('whalliam-hidden', !visible);
  }
  ...
  private render() {
    ...
    const gaugeEl = this.container.createDiv({ cls: 'whalliam-context-meter-gauge' });
    ...
    backgroundPath.classList.add('whalliam-meter-bg');
    ...
    fillPath.classList.add('whalliam-meter-fill');
    ...
    this.percentEl = this.container.createSpan({ cls: 'whalliam-context-meter-percent' });
  }
