...
export function autoResizeTextarea(textarea: HTMLTextAreaElement): void {
  const viewHeight = textarea.closest('.whalliam-container')?.clientHeight ?? window.innerHeight;
  const maxHeight = calculateTextareaMaxHeight(viewHeight);

  textarea.setCssProps({
    '--whalliam-textarea-min-height': `${TEXTAREA_BASE_MIN_HEIGHT}px`,
    '--whalliam-textarea-max-height': `${maxHeight}px`,
  });
  ...
  textarea.setCssProps({
    '--whalliam-textarea-min-height': `${minHeight}px`,
    '--whalliam-textarea-max-height': `${maxHeight}px`,
  });
}
