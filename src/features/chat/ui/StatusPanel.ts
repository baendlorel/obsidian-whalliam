...
export class StatusPanel {
  ...
  private createPanel(): void {
    ...
    this.panelEl.className = 'whalliam-status-panel';

    // Bash output container - hidden by default
    this.bashOutputContainerEl = ownerDocument.createElement('div');
    this.bashOutputContainerEl.className = 'whalliam-status-panel-bash whalliam-hidden';

    this.bashHeaderEl = ownerDocument.createElement('div');
    this.bashHeaderEl.className = 'whalliam-tool-header whalliam-status-panel-bash-header';
    ...
    this.bashContentEl.className = 'whalliam-status-panel-bash-content';
    ...
    this.todoContainerEl.className = 'whalliam-status-panel-todos whalliam-hidden';
    this.panelEl.appendChild(this.todoContainerEl);

    // Todo header (collapsed view)
    this.todoHeaderEl = ownerDocument.createElement('div');
    this.todoHeaderEl.className = 'whalliam-status-panel-header';
    ...
    this.todoContentEl.className = 'whalliam-status-panel-content whalliam-todo-list-container whalliam-hidden';
  ...
  updateTodos(todos: TodoItem[] | null): void {
    ...
    if (!todos || todos.length === 0) {
      this.todoContainerEl.addClass('whalliam-hidden');
      this.todoHeaderEl.empty();
      this.todoContentEl.empty();
      return;
    }

    this.todoContainerEl.removeClass('whalliam-hidden');
  ...
  private renderTodoHeader(completedCount: number, totalCount: number, currentTask: TodoItem | undefined): void {
    ...
    icon.className = 'whalliam-status-panel-icon';
    setIcon(icon, getToolIcon(TOOL_TODO_WRITE));
    this.todoHeaderEl.appendChild(icon);

    // Label
    const label = ownerDocument.createElement('span');
    label.className = 'whalliam-status-panel-label';
    ...
    if (!this.isTodoExpanded) {
      ...
      if (completedCount === totalCount && totalCount > 0) {
        ...
        status.className = 'whalliam-status-panel-status status-completed';
      ...
      if (currentTask) {
        ...
        current.className = 'whalliam-status-panel-current';
  ...
  private updateTodoDisplay(): void {
    ...
    // Show/hide content
    this.todoContentEl.toggleClass('whalliam-hidden', !this.isTodoExpanded);
  ...
  private renderBashOutputs(options: { scroll?: boolean } = {}): void {
    ...
    if (this.currentBashOutputs.size === 0) {
      this.bashOutputContainerEl.addClass('whalliam-hidden');
      return;
    }

    this.bashOutputContainerEl.removeClass('whalliam-hidden');
    ...
    headerIconEl.className = 'whalliam-tool-icon';
    ...
    headerLabelEl.className = 'whalliam-tool-label';
    ...
    previewEl.className = 'whalliam-tool-current';
    previewEl.classList.toggle('whalliam-hidden', !this.isBashExpanded);
    this.bashHeaderEl.appendChild(previewEl);

    const summaryStatusEl = ownerDocument.createElement('span');
    summaryStatusEl.className = 'whalliam-tool-status';
    if (!this.isBashExpanded && latest) {
      ...
      summaryStatusEl.classList.add('whalliam-hidden');
    }
    ...
    actionsEl.className = 'whalliam-status-panel-bash-actions';
    ...
    this.bashContentEl.toggleClass('whalliam-hidden', !this.isBashExpanded);
  ...
  private renderBashEntry(info: PanelBashOutput, ownerDocument: Document): HTMLElement {
    ...
    entryEl.className = 'whalliam-tool-call whalliam-status-panel-bash-entry';

    const entryHeaderEl = ownerDocument.createElement('div');
    entryHeaderEl.className = 'whalliam-tool-header';
    ...
    entryIconEl.className = 'whalliam-tool-icon';
    ...
    entryLabelEl.className = 'whalliam-tool-label';
    ...
    entryStatusEl.className = 'whalliam-tool-status';
    ...
    contentEl.className = 'whalliam-tool-content';
    const isEntryExpanded = this.bashEntryExpanded.get(info.id) ?? true;
    contentEl.classList.toggle('whalliam-hidden', !isEntryExpanded);
    ...
    rowEl.className = 'whalliam-tool-result-row';

    const textEl = ownerDocument.createElement('span');
    textEl.className = 'whalliam-tool-result-text';
  ...
  private appendActionButton(
    ...
    el.className = `whalliam-status-panel-bash-action whalliam-status-panel-bash-action-${name}`;
